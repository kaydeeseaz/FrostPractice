package dev.demeng.frost.scoreboard;

import static dev.demeng.frost.scoreboard.AetherOptions.defaultOptions;
...
import dev.demeng.frost.scoreboard.scoreboard.BoardEntry;
          ...
          if (board != null) {
            ...
            for (int i = 0; i < scores.size(); i++) {
              ...
              while (iterator.hasNext()) {
                ...
                if (score != null
                    && boardEntry.getText().equals(CC.color(text))
              ...
              while (iterator.hasNext()) {
                BoardEntry boardEntry = iterator.next();
                int entryPosition = scoreboard.getObjective(DisplaySlot.SIDEBAR)
                    .getScore(boardEntry.getKey()).getScore();
              ...
              if (board.getEntries().size() > scores.size()) {
                ...
                while (iterator.hasNext()) {
                  BoardEntry boardEntry = iterator.next();
                  if ((!translatedScores.contains(boardEntry.getText())) ||
                      Collections.frequency(board.getBoardEntriesFormatted(), boardEntry.getText())
                          > 1) {
                    iterator.remove();
                    boardEntry.remove();
                  }
