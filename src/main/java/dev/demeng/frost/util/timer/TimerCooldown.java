...
import dev.demeng.frost.util.timer.event.TimerExpireEvent;
...
public class TimerCooldown {
  ...
  @Getter private long expiryMillis;

  @Getter @Setter(AccessLevel.PROTECTED) private long pauseMillis;
        ...
        public void run() {
          if (TimerCooldown.this.timer instanceof PlayerTimer && owner != null) {
            ((PlayerTimer) timer).handleExpiry(
                Frost.getInstance().getServer().getPlayer(TimerCooldown.this.owner),
                TimerCooldown.this.owner);
          }
