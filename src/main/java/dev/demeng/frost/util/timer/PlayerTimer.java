package dev.demeng.frost.util.timer;

import dev.demeng.frost.Frost;
import dev.demeng.frost.util.timer.event.TimerClearEvent;
import dev.demeng.frost.util.timer.event.TimerExtendEvent;
import dev.demeng.frost.util.timer.event.TimerPauseEvent;
import dev.demeng.frost.util.timer.event.TimerStartEvent;
...
public abstract class PlayerTimer extends Timer {
  ...
  @Getter protected final Map<UUID, TimerCooldown> cooldowns = new ConcurrentHashMap<>();
  ...
  public TimerCooldown clearCooldown(Player player, UUID playerUUID) {
    ...
    if (runnable != null) {
      runnable.cancel();
      if (player == null) {
        Frost.getInstance().getServer().getPluginManager()
            .callEvent(new TimerClearEvent(playerUUID, this));
      } else {
        Frost.getInstance().getServer().getPluginManager()
            .callEvent(new TimerClearEvent(player, this));
      }
    }
  ...
  public boolean setCooldown(Player player, UUID playerUUID, long duration, boolean overwrite,
    ...
    TimerCooldown runnable =
        duration > 0L ? this.cooldowns.get(playerUUID) : this.clearCooldown(player, playerUUID);
    if (runnable != null) {
      ...
      Frost.getInstance().getServer().getPluginManager().callEvent(event);
      if (event.isCancelled()) {
        return false;
      }
      ...
      Frost.getInstance().getServer().getPluginManager()
          .callEvent(new TimerStartEvent(player, playerUUID, this, duration));
