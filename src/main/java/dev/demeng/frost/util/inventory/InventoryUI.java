package dev.demeng.frost.util.inventory;
...
public class InventoryUI {
  ...
  @Getter private final List<Inventory2D> inventories = new LinkedList<>();
  ...
  private void createNewInventory() {
    ...
    if (this.inventories.size() > 0) {
      inventory.setItem(0, this.rows - 1, new AbstractClickableItem(
          ItemUtil.createItem(Material.ARROW,
              ChatColor.AQUA + "Page #" + (this.inventories.size()))) {
        @Override
  ...
  public void addItem(ClickableItem item) {
    ...
    if (lastInventory.currentY == this.rows - 1 && lastInventory.currentX >= 7 - this.offset) {
      lastInventory.setItem(8, this.rows - 1, new AbstractClickableItem(
          ItemUtil.createItem(Material.ARROW,
              ChatColor.RED + "Page #" + (this.inventories.size() + 1))) {
        @Override
        public void onClick(InventoryClickEvent event) {
          InventoryUI.this.page++;
          try {
  ...
  public class Inventory2D {
    ...
    public void setItem(int x, int y, ClickableItem clickableItem) {
      ...
      if (this.cachedInventory != null) {
        ...
        this.cachedInventory.setItem(slot,
            clickableItem != null ? clickableItem.getItemStack() : null);
      }
    }
    ...
    public Inventory toInventory() {
      ...
      Inventory inventory = Bukkit.getServer()
          .createInventory(new InventoryUIHolder(InventoryUI.this, this.title, this.rows * 9),
              this.rows * 9, this.title);
